...
\begin{center}
  ...
  \begin{lstlisting}[frame=single]  % Start your code-block
    ...
    #fitness funkce, která počítá vzdálenost mezi vloženým vektorem a solution 
    def f(w): return -np.sum((w - solution)**2)
    
    npop = 50      # velikost populace
    sigma = 0.1    # parametry ES
    alpha = 0.001  # learning rate
    w = np.random.randn(3) # první vektor    
    for i in range(300): #udělejme 300 iterací
      N = np.random.randn(npop, 3) #vygenerujme npop mutací k aktuálním řešení
      R = np.zeros(npop) # vektor R slouží k uchování fitness hodnot
      for j in range(npop): #Pro každou vyrobenou mutaci
        w_try = w + sigma*N[j] #Aplikujeme mutaci na aktuálního jedince
        R[j] = f(w_try) #Ohodnotíme úspěšnost zmutovaného jedince vzhledem k fitness funkci
      A = (R - np.mean(R)) / np.std(R) 
      w = w + alpha/(npop*sigma) * np.dot(N.T, A) # upravíme aktuální řešení mutacemi, čím úspěšnější mutace, tím větší vliv do aktuálního jedince.
  \end{lstlisting}
